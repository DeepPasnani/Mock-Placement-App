...
import { Clock, Play, CheckCircle, Lock, Calendar } from 'lucide-react';
...
export default function StudentTests() {
  ...
  if (loadingTests) return (
    <div className="flex justify-center py-24"><Spinner size={36} className="text-brand" /></div>
  );
    ...
    <div>
      ...
      {tests.length === 0 && (
        <div className="bg-white border border-gray-200 rounded-2xl p-16 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-600 mb-1">No tests available</h3>
          <p className="text-gray-400 text-sm">Tests published by your placement coordinator will appear here.</p>
        </div>
      )}
            ...
            <div key={test.id} className="bg-white border border-gray-200 rounded-2xl p-6 hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {(test.sections || []).map(sec => (
                      <span key={sec.id} className={`text-xs font-medium px-2.5 py-1 rounded-full ${sec.type === 'coding' ? 'bg-blue-50 text-blue-700' : 'bg-purple-50 text-purple-700'}`}>
                ...
                <div className="shrink-0 text-right">
                  {submitted ? (
                    <div>
                      <div className={`text-3xl font-extrabold mb-0.5 ${passed ? 'text-accent' : 'text-red-500'}`}>
                        {pct}%
                      </div>
                      <div className="text-xs text-gray-400 mb-2">{sub.score}/{sub.max_score} marks</div>
                      <ProgressBar value={sub.score} max={sub.max_score} color={passed ? 'bg-accent' : 'bg-red-400'} className="w-28" />
                    </div>
                  ) : inProgress ? (
